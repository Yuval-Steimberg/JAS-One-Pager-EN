...
import { ACTIVITIES, CONTACT } from "@/content/site";
            ...
            >
              ...
              <div className={cn(reversed ? "lg:order-1" : "lg:order-2")}>
                ...
                <dl className="space-y-4">
                  {FIELDS.map((field) => (
                    <motion.div
                      key={field.key}
                      variants={fadeUp}
                      className="border-s-2 border-ember/30 ps-4"
                    >
                      <dt className="text-xs font-semibold uppercase tracking-wider text-ember">
                        {field.label}
                      </dt>
                      <dd className="mt-0.5 text-base leading-relaxed text-forest/75">
                        {activity[field.key]}
                      </dd>
                    </motion.div>
                  ))}
                ...
                <motion.a
                  ...
                  className="group mt-7 inline-flex items-center gap-2 rounded-full bg-forest px-6 py-3 text-sm font-semibold text-cream transition-colors hover:bg-forest-700"
                >
                  למידע נוסף
