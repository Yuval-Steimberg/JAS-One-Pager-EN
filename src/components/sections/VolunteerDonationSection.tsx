...
import { VOLUNTEER } from "@/content/site";
...
export function VolunteerDonationSection() {
  return (
    <section
      ...
      className="on-dark relative scroll-mt-[72px] overflow-hidden bg-ember py-20 text-white sm:py-28 lg:py-32"
    >
            ...
            return (
              <motion.div key={card.title} variants={fadeUp}>
                <TiltCard3D className="h-full" max={6} glare={false}>
                  <article className="flex h-full flex-col rounded-xl2 bg-cream p-7 text-forest shadow-lift">
                    <span className="mb-5 flex h-14 w-14 items-center justify-center rounded-full bg-ember/10 text-ember">
                      <Icon className="h-7 w-7" />
                    </span>
                    <h3 className="mb-2 text-xl font-bold">{card.title}</h3>
                    <p className="mb-6 flex-1 text-sm leading-relaxed text-forest/65">
                      {card.text}
                    </p>
                    <a
                      href={card.cta.href}
                      ...
                      className="group inline-flex items-center gap-2 self-start rounded-full bg-forest px-5 py-2.5 text-sm font-semibold text-cream transition-colors hover:bg-forest-700"
                    >
                      {card.cta.label}
