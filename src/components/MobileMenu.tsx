    ...
    <AnimatePresence>
      {open && (
        ...
        >
          ...
          <motion.nav
            variants={panel}
            className="absolute inset-y-0 end-0 flex w-[82%] max-w-sm flex-col bg-cream shadow-lift"
          >
            <div className="flex items-center justify-between border-b border-forest/10 px-6 py-5">
              ...
              <button
                ...
                onClick={onClose}
                aria-label="סגירת התפריט"
                className="rounded-full p-2 text-forest transition-colors hover:bg-forest/5"
              >
                <X className="h-6 w-6" />
              </button>
            </div>
            ...
            <div className="mt-auto border-t border-forest/10 p-6">
              ...
              <p className="mt-4 text-center text-sm text-forest/50">{CONTACT.address}</p>
            </div>
          </motion.nav>
        </motion.div>
      )}
