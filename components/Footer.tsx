...
import { FOOTER, CONTACT, BRAND, asset } from "@/data/siteContent";
...
export function Footer() {
  ...
  return (
    <footer className="border-t border-ivory/10 bg-ink-900 text-ivory">
      <div className="container py-16">
        <div className="grid gap-10 md:grid-cols-2 lg:grid-cols-4">
          ...
          <nav aria-label="קישורים מהירים">
            <h3 className="mb-4 text-sm font-bold uppercase tracking-wider text-ivory/80">
              ניווט
            </h3>
          ...
          <div>
            ...
            <ul className="space-y-3 text-sm text-ivory/55">
              ...
              <li className="flex items-center gap-2.5">
                <MessageCircle className="h-4 w-4 shrink-0 text-clay" />
                <a href={CONTACT.whatsapp} dir="ltr" className="transition-colors hover:text-ivory">
                  {CONTACT.whatsappDisplay}
                </a>
              </li>
              <li className="flex items-center gap-2.5">
                <Mail className="h-4 w-4 shrink-0 text-clay" />
                <a href={`mailto:${CONTACT.email}`} dir="ltr" className="transition-colors hover:text-ivory">
                  {CONTACT.email}
                </a>
              </li>
            </ul>
        ...
        <div className="mt-12 flex flex-col items-center justify-between gap-4 border-t border-ivory/10 pt-8 text-xs text-ivory/40 sm:flex-row">
          ...
          <ul className="flex gap-5">
            {FOOTER.legal.map((item) => (
              <li key={item.label}>
                <a href={item.href} className="transition-colors hover:text-ivory/70">
                  {item.label}
                </a>
              </li>
            ))}
...
function SocialIcon({
  ...
  return (
    <a
      ...
      aria-label={label}
      className="flex h-10 w-10 items-center justify-center rounded-full border border-ivory/15 text-ivory/70 transition-colors hover:border-clay hover:bg-clay hover:text-white"
    >
      {children}
    </a>
  );
